...
The major shader types are vertex and fragment shaders. Vertex shaders apply to every element of a user-given array. The shader is written to calculate a vertex, a coordinate. The vertex is used for rasterization. Rasterization turns every 3 vertices into a triangle.
In the fragment shader, for every drawn fragment, also called pixel, is a user-defined function applied. The shader generally applies textures to fragments and delivers a rendered image.
